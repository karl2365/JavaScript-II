...
let largeShirts = [];
    largeShirts = runners.filter((r) => r.shirt_size =="L");
console.log(largeShirts);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations into a ticketPriceTotal array and log the result
let ticketPriceTotal = []
    runners.reduce(function(acc, run){
        ticketPriceTotal.push(run.donation);
    },0)
...
// Problem 1
let totalDonations = runners.reduce(function(acc, run){
    acc += run.donation;
    return acc;
}, 0)
...
let nameCompany = [];
runners.forEach((el, idx) => {
    fullName.push(`${el.first_name} ${el.last_name} is from ${el.company_name}`);
    })
console.log(nameCompany);

// Problem 3
let bigDonors = [];
    bigDonors = runners.filter((r) => r.donation > 200);
